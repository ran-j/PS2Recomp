...
enum class ThemeMode {
    Dark, Light, Custom, Stylized, PS2Classic
};
...
public:
    ...
    void ScanFonts() {
        ...
        try {
            std::filesystem::path fontDir = "ps2xStudio/external/Font";
            if (!std::filesystem::exists(fontDir)) {
                fontDir = "external/Font";
            }
            if (std::filesystem::exists(fontDir)) {
    ...
    void LoadConfigToml() {
            ...
            try {
                ...
                if (file && file.good()) {
                    ...
                    if (!content.empty()) {
                        ...
                        // Sync UI state from TOML
                        try {
                            ps2recomp::ConfigManager mgr(configTomlPath);
                            auto cfg = mgr.loadConfig();
                            if (!cfg.outputPath.empty()) {
                                data.customOutputPath = cfg.outputPath;
                            }
                        } catch(...) {}
        ...
        if (!loaded) {
            ...
            // Sync UI state from newly created TOML
            try {
                ps2recomp::ConfigManager mgr(configTomlPath);
                auto cfg = mgr.loadConfig();
                if (!cfg.outputPath.empty()) {
...
stubs = []
    ...
    void SaveConfigTOML() {
        ...
        try {
            ...
            // Ensure editor content is saved first so we can parse it and preserve custom fields
            ...
            config.stubImplementations.clear();
            config.skipFunctions.clear();
