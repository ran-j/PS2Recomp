...
{
    MiniTest::Case("Decoder", [](TestCase &tc)
                   {
    tc.Run("decodes JAL with jump target and call flag", [](TestCase &t) {
        ...
        Instruction inst = decodeInstruction(address, raw);
        ...
        t.Equals(getJumpTarget(inst), target, "jal jump target should match encoded target");
    });
    ...
    tc.Run("J computes target with upper PC bits", [](TestCase &t) {
        ...
        t.Equals(getJumpTarget(inst), expected, "J target should combine PC upper bits with encoded target");
    });
    ...
    tc.Run("JR/JALR jump target is zero (dynamic)", [](TestCase &t) {
        ...
        Instruction jr = decodeInstruction(address, jrRaw);
        Instruction jalr = decodeInstruction(address + 4, jalrRaw);

        t.Equals(getJumpTarget(jr), 0u, "JR jump target should be unknown (0)");
        t.Equals(getJumpTarget(jalr), 0u, "JALR jump target should be unknown (0)");
    });
    ...
    tc.Run("decodes BEQ sets branch flags and target", [](TestCase &t) {
        ...
        t.Equals(getBranchTarget(inst), expectedTarget, "beq target should be computed from simmediate");
    });
    ...
    tc.Run("branch target sign-extends negative offset", [](TestCase &t) {
        ...
        t.Equals(getBranchTarget(inst), expectedTarget, "negative branch offsets should sign-extend");
    });
    ...
    tc.Run("decodes load/store flags", [](TestCase &t) {
        ...
        Instruction lw = decodeInstruction(address, lwRaw);
        Instruction sw = decodeInstruction(address + 4, swRaw);
    ...
    tc.Run("REGIMM branch and link marks call and GPR modification", [](TestCase &t) {
        ...
        uint32_t expectedTarget = address + 4 + (static_cast<int16_t>(offset) << 2);
        t.Equals(getBranchTarget(inst), expectedTarget, "bgezal target should be computed");
    });
    ...
    tc.Run("LL/SC modify control and set load/store flags", [](TestCase &t) {
        ...
        Instruction ll = decodeInstruction(address, llRaw);
        Instruction sc = decodeInstruction(address + 4, scRaw);
