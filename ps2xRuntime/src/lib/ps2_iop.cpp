...
namespace
{
    constexpr uint32_t kFatalFrameImgHeaderAddr = 0x012F0000u;
    constexpr uint32_t kFatalFrameSectorSize = 2048u;
    constexpr uint32_t kFatalFrameStatusOffset = 0x6Cu;
    constexpr uint32_t kFatalFrameStatusStride = 8u;
    ...
    bool loadFatalFrameImageHeader(uint8_t *rdram)
    {
        ...
        uint8_t *dst = getMemPtr(rdram, kFatalFrameImgHeaderAddr);
        if (!dst)
        {
            return false;
        }

        const size_t bytes = static_cast<size_t>(std::min<uint64_t>(fileBytes, PS2_RAM_SIZE - (kFatalFrameImgHeaderAddr & PS2_RAM_MASK)));
        return readHostRange(headerPath, 0u, dst, bytes);
    }

    void warnFatalFrameImageHeaderLoadFailed()
    {
        static uint32_t s_headerWarnCount = 0u;
        if (s_headerWarnCount < 4u)
        {
    ...
    bool readFatalFrameBody(uint8_t *rdram, uint32_t lbn, uint32_t byteCount, uint32_t dstAddr)
    {
        if (byteCount == 0)
        {
            return true;
        ...
        const uint64_t offsetBytes = static_cast<uint64_t>(lbn) * kFatalFrameSectorSize;
        return readHostRange(bodyPath, offsetBytes, dst, bytes);
    }

    void clearFatalFrameRecv(uint8_t *rdram, uint32_t recvBufAddr, uint32_t recvSize)
    {
    ...
    void markFatalFrameLoadComplete(uint8_t *rdram, uint32_t recvBufAddr, uint32_t recvSize, uint32_t loadId)
    {
        const uint32_t statusOffset = kFatalFrameStatusOffset + ((loadId & 0x1Fu) * kFatalFrameStatusStride);
    ...
    bool handleFatalFrameSdrdrvRpc(uint8_t *rdram,
                                   ...
                                   uint32_t &resultPtr)
    {
        ...
        clearFatalFrameRecv(rdram, recvBufAddr, recvSize);

        if (rpcNum == 0u)
        {
            if (!loadFatalFrameImageHeader(rdram))
            {
                warnFatalFrameImageHeaderLoadFailed();
            }
            return true;
        }
        ...
        for (uint32_t commandIndex = 0; commandIndex < commandCount; ++commandIndex)
        {
            ...
            case 0x0Eu:
            {
                ...
                const bool bodyLoaded = !eeLoad || readFatalFrameBody(rdram, lbn, byteCount, dstAddr);
                ...
                if (eeLoad && !bodyLoaded)
                {
                    ...
                    static uint32_t s_bodyWarnCount = 0u;
                    if (s_bodyWarnCount < 8u)
                    {
                ...
                markFatalFrameLoadComplete(rdram, recvBufAddr, recvSize, loadId);
                break;
            }
                        ...
                        bool &signalNowaitCompletion)
{
    ...
    if (handleFatalFrameSdrdrvRpc(m_rdram,
                                  sid, rpcNum,
                                  sendBufAddr, sendSize,
                                  recvBufAddr, recvSize,
                                  resultPtr))
