...
namespace
{
                                ...
                                uint32_t &outPixel)
    {
        ...
        const uint32_t bytesPerPixel =
            (psm == GS_PSM_CT16 || psm == GS_PSM_CT16S) ? 2u :
            (psm == GS_PSM_CT32 || psm == GS_PSM_CT24) ? 4u :
                                                       0u;
        if (bytesPerPixel == 0u)
        {
            return false;
    ...
    {
        if (!runtime || !runtime->ensureCoreSubsystemsInitialized() || !hasSeededGsClearPacket(clear))
        {
            return;
        }
...
void sceGsExecLoadImage(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
{
    ...
    GsImageMem img{};
    if (!runtime || !runtime->ensureCoreSubsystemsInitialized() || !readGsImage(rdram, imgAddr, img))
    {
        setReturnS32(ctx, -1);
        return;
    }
